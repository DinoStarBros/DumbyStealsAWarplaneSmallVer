...
var direction_to_target_deviate_pos : Vector2
...
func behave(delta: float) -> void:
	...
	direction_to_target_deviate_pos = global_position.direction_to(target.global_position + target_deviation)
	...
	parent.velocity_component.other_velocity_handle(delta, direction_to_target_deviate_pos, true)
